...
class Employee_model extends CI_Model {
	...
	function getDailyRate_from_SalaryTable($payperiod_obj, $empnum)
	{
		...
		if( empty($array_result) ) return $this->ErrorReturn_model->createSingleError(409, $empnum, NULL);
		else{
			//invalid daily rate error
			if( ($array_result[0]->DailyRate > 0) == FALSE ) return $this->ErrorReturn_model->createSingleError(456, $empnum, NULL);		
						
			return $this->ErrorReturn_model->createSingleError(0, $array_result[0]->DailyRate, NULL);		
		}
	}
	
	function getAllEmployees_DailyRate($payperiod_obj = NULL)
	{
		/*
			made | abe | 15JUN2011_2201
			returns an array generated by ErrorReturn_model's CreateSingleError(..)
			the function's over-all doing is determined by the outer main array.
			the results (employees' daily rate or the bunch of errors) are on the
			main array's 'FURTHER_INFO' index
		*/
		$dailyRateErrorOnSalary = FALSE;
		$this_errors = array();
		
		if($payperiod_obj == NULL)
		{
			return $this->ErrorReturn_model->createSingleError(452, NULL, NULL);				
		}
		
		$currentEmployees = $this->get_Employees_Associative();
		...
		{			
			...
			{
				$dailyRateErrorOnSalary = TRUE;
				$this_errors[] = $xyza;
			}
		    $currentEmployeesDailyRate[$eachEmployee->empnum] = floatval($xyza['FURTHER_INFO']);
		}	
		
		if($dailyRateErrorOnSalary) 
			return $this->ErrorReturn_model->createSingleError(1000, $this_errors, NULL);				
		
		if(empty($currentEmployeesDailyRate))
			return $this->ErrorReturn_model->createSingleError(101, NULL, NULL);				
		else			
			return $this->ErrorReturn_model->createSingleError(0, $currentEmployeesDailyRate, NULL);		
	}
