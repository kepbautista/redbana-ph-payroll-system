...
/*
 ...
 FilePurpose: MODEL-SNIFFBROWSER
 version: 1.0.0

 DISCLAIMER: Most parts of this code were copied from a file downloadable from 
			 http://techpatterns.com/downloads/php_browser_detection.php (downloaded by 01 MAY 2011).
			 The owner of the file (see below) allows use to use the file (and/or its code) for
			 the Payroll System as per GNU GPL v3.
 
 CHANGELOG
 01may2011 1538h - 1.0.0 - basic creation. initial release. basic infos only outputted. 

*/

class sniffBrowser_model extends CI_Model{
	...
	*/
	...
	Special thanks to alanjstr for cleaning up the code, especially on private function get_item_version(),
	...
	******************************************
	...
	This change will will NOT break your existing programming or browser detection private function calls.
	******************************************
	...
	******************************************
	...
	argument used in the private function call if used, like: browser_detection( 'full', '1' );
	...
	*******************************************/
	...
	// main script, uses two other private functions, get_os_data() and get_item_version() as needed
		...
		*/
		if ( !$b_repeat )
		{
			...
			*/
					...
					switch ( $browser_working )
					{
						...
						case 'moz':
							...
							*/
							...
							// gets the actual release date, necessary if you need to do private functionality tests
			...
			to true so the private function won't retest everything
			*/
			$b_repeat = true;
			/*
		...
		/*
		This is where you return values based on what parameter you used to call the private function
	...
	private function get_os_data ( $pv_browser_string, $pv_browser_name, $pv_version_number  )
	{
		...
		*/
		...
		// pack the os data array for return to main private function
	...
	*/
	private function check_is_mobile( $pv_browser_user_agent )
	{
		...
		This list is almost the same but not exactly as the 4 arrays in private function below
		*/
	...
	*/
	...
	function isBrowser_Acceptable($sniffedData = NULL)
	{
		/*
			this function is self-titled. :)
			anyway, the $allowedBrowsers array should be changed when the need arises.								
		*/
		$allowedBrowsers = array(
						   "msie" => "7.0",
						   "firefox" => "2.0"
		);
		...
		if( $sniffedData!=NULL )
		{			
		    if($sniffedData["browser_name"] != "unknown")
			{
				//no info on restriction  on the detected browser so far, so ok
				if(!isset($allowedBrowsers[$sniffedData["browser_name"]]) ) return TRUE;     
				
				//since browser numbers are often in the format of at least X.Y (e.g. 11.1), tokenize, use the first two
				$compare_from_detection = explode(".", $sniffedData["browser_version"]);							
				$compare_on_record = explode(".", $allowedBrowsers[$sniffedData["browser_name"]]);				
				
				if( $compare_from_detection[0] >=  $compare_on_record[0] &&   
					$compare_from_detection[1] >=  $compare_on_record[1]
				   )				
					return TRUE;
				else
					return FALSE;
	...
	{
		/*
			returns an array with the ff fields:
			
			"browser_name" 		:	'msie', 'firefox', 'chrome', 'safari', 'opera'
			"browser_version"	:   in the format of at least X.Y e.g. 7.0, 11.0.696, however only the first two figures are considered
			"ie_version"		:	might be useful when considering IE's 'Compatibility Mode'
			"true_ie_number"	:	might be useful when considering IE's 'Compatibility Mode'
			"os_type" 			:	so far, only tried in Windows XP-7 value is 'nt'
			"os_number" 		:   so far, only tried in Windows XP-7 values are '5.1', '6.0', '6.1', 
		*/
	
		//let's perform the dirty work
		$from_browser_detection = $this->browser_detection( 'full', '', '' ) ;			
		
		// let's make an associative array for easy manipulation and information-sharing		
		static $sniffedData = array( "browser_name" => NULL, 
		...
		/*
			It seems that for all latest browsers to date 02 MAY 2011,
			no difference in determining OS.
			What do I mean by difference?
			E.g. in this script, in IE, the basic browser info like name
			and version are already in the 'main outside array', the returned
			array from browser_detection(..), however for others,
			such info are stored on another array located at browser_detection(..)[10]
			which needs more methods.
			
			So, update the OS fields first.
		*/
		$sniffedData["os_type"] = $from_browser_detection[5];
		$sniffedData["os_number"] = $from_browser_detection[6];
		
		switch( $from_browser_detection[7] )  		//the 'browser name'
		{
			case "msie":
						$sniffedData["browser_name"] = "msie";
						$sniffedData["browser_version"] =  $from_browser_detection[1];
						$sniffedData["true_ie_number"] = $from_browser_detection[14];
						break;
			case "opera":
						$sniffedData["browser_name"] = "opera";
						$sniffedData["browser_version"] =  $from_browser_detection[1];
						break;
			case  "safari":
						$sniffedData["browser_name"] = "safari";
						$sniffedData["browser_version"] =  $from_browser_detection[1];
						break;
			case "chrome":
						$sniffedData["browser_name"] = "chrome";
						$sniffedData["browser_version"] =  $from_browser_detection[10][1];
						break;
			case "gecko":	//often in firefox
						$sniffedData["browser_name"] = $from_browser_detection[10][0];
						$sniffedData["browser_version"] =  $from_browser_detection[10][1];					
						break;
			default: 
						$sniffedData["browser_name"] = "unknown";
						break;
		}
...
/* End of file sniffBrowser_model.php */
/* Location: ./application/models/sniffBrowser_model.php */
?>